...
import {
  Box, Flex, HStack, VStack, Text, Button, Icon,
  Divider, Container,
        ...
        <HStack justify="space-between" align="center" flexWrap="wrap" gap={6}>
          ...
          <HStack spacing={6} flexWrap="wrap">
            {[
              ...
              { href: '/compare/freelance-finance-spreadsheet-vs-app', label: 'Spreadsheet vs App' },
              { href: APP_URL,                                       label: 'Sign in' },
            ].map(l => (
              <Text
                key={l.href}
                as="a" href={l.href}
                fontSize="13px" color="rgba(157,143,212,0.6)" fontWeight="500"
                _hover={{ color: 'white' }} transition="color 0.15s"
                textDecoration="none"
              >
                {l.label}
...
export function InlineCTA({
  ...
  return (
    ...
    >
      <Box
        position="absolute" top="-80px" right="-80px" w="300px" h="300px"
        borderRadius="full"
        bg="radial-gradient(circle, rgba(124,92,246,0.22) 0%, transparent 70%)"
        pointerEvents="none"
      />
      <Box
        position="absolute" bottom="-60px" left="-60px" w="250px" h="250px"
        borderRadius="full"
      ...
      <Text
        ...
        color="white" mb={3} position="relative"
      >
        {headline}
      </Text>
      <Text
        fontSize="15px" color="#9d8fd4"
        lineHeight="1.65" mb={6}
        maxW="440px" mx="auto" position="relative"
      >
        {sub}
      </Text>
      <Button
        as="a" href={`${APP_URL}?ref=${utmRef}`}
        bg={BRAND} color="white"
        fontWeight="700" fontSize="15px"
        borderRadius="10px" h="48px" px={8}
      ...
      <Text fontSize="12px" color="rgba(157,143,212,0.45)" mt={3} position="relative">
    ...
    <VStack spacing={3} align="stretch" my={5}>
      {items.map(item => (
        <HStack key={item} spacing={3} align="flex-start">
          ...
          <Text fontSize="14px" color="#475569" lineHeight="1.7">{item}</Text>
        </HStack>
      ))}
    </VStack>
  );
...
export function RelatedPages({ links }: { links: { href: string; label: string; desc: string }[] }) {
  return (
    <Box mt={10}>
      <Divider mb={6} borderColor="#E8E8E3" />
      <Text
        fontSize="11px" fontWeight="700" color="#8a9aaa"
        textTransform="uppercase" letterSpacing="1.2px" mb={4}
      >
        Related guides
      </Text>
      <VStack spacing={3} align="stretch">
        {links.map(l => (
          <Box
            ...
            as="a" href={l.href}
            display="block" p={4}
            bg="white" border="1px solid #E8E8E3" borderRadius="12px"
            _hover={{ borderColor: BRAND, bg: '#f8f9ff', transform: 'translateX(2px)' }}
            transition="all 0.15s" textDecoration="none"
          >
            <Text fontSize="14px" fontWeight="600" color={BRAND} mb={0.5}>{l.label}</Text>
            <Text fontSize="13px" color="#5a6a7a">{l.desc}</Text>
          </Box>
        ))}
      </VStack>
    </Box>
...
// ─── ArticleHeader ────────────────────────────────────────────────────────────
export function ArticleHeader({ tag, readTime, h1, lead }: {
  tag: string; readTime: string; h1: string; lead: string;
}) {
  return (
    <Box mb={8}>
      <HStack spacing={2} mb={4}>
        <Box
          px={3} py={0.5}
          bg="rgba(76,95,213,0.1)" border="1px solid rgba(76,95,213,0.2)"
          borderRadius="full"
        >
          <Text fontSize="11px" fontWeight="700" color={BRAND}
            textTransform="uppercase" letterSpacing="0.8px">
            {tag}
          </Text>
        </Box>
        <Text fontSize="13px" color="#8a9aaa">{readTime}</Text>
      </HStack>
      <Text
        as="h1"
        fontFamily="'Fraunces', Georgia, serif"
        ...
        color="#1C2B3A" lineHeight="1.15" mb={4}
      >
        {h1}
      </Text>
      <Text fontSize="17px" color="#5a6a7a" lineHeight="1.7">{lead}</Text>
    </Box>
  );
}
...
export function P({ children }: { children: React.ReactNode }) {
  return <Text fontSize="15px" color="#475569" lineHeight="1.8" mb={4}>{children}</Text>;
}
...
export function SEOPageLayout({
  ...
  return (
    <Box bg="#F5F4EF" minH="100vh" fontFamily="'DM Sans', -apple-system, sans-serif">
      <SEONavbar />
